...
class ModerationPipeline:
    def __init__(self, model_path="./distilbert-toxic-mitigated"):
        ...
        # Load the model but don't fit as a classifier - we'll use cv=3 for internal calibration
        self.hf_wrapper.load_model()
        self.hf_wrapper.classes_ = np.array([0, 1])
        
        # Use cv=3 for internal cross-validation during calibration
        # This fits the model on 2/3 of the data and calibrates on 1/3, repeated 3 times
        self.calibrator = CalibratedClassifierCV(estimator=self.hf_wrapper, method='isotonic', cv=3)
        self.is_calibrated = False

    def fit_calibrator(self, val_texts, val_labels):
        """Fit the probability calibrator using validation data."""
        # For cv=3, we need to provide X and y for the internal cross-validation
        # The calibrator will split this data internally
    ...
    def predict_batch(self, texts: list) -> list:
        """Run the three layers in sequence on a batch of texts."""
        results = []
        for text in texts:
            # Layer 1: Input filter
            filter_result = self.filter(text)
            ...
            if prob >= 0.6:
                results.append({"decision": "block", "layer": "model", "confidence": float(prob)})
            elif prob <= 0.4:
                results.append({"decision": "allow", "layer": "model", "confidence": float(prob)})
            else:
                results.append({"decision": "review", "layer": "model", "confidence": float(prob)})
